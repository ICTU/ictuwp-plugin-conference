 ...
 *
 * @wordpress-plugin
 * Plugin Name:         ICTU / Gebruiker Centraal / Conference post types and
 *   taxonomies Plugin URI:
 *   https://github.com/ICTU/Gebruiker-Centraal---Inclusie---custom-post-types-taxonomies
 *   Description:         Plugin for conference.gebruikercentraal.nl to
 *   register custom post types and custom taxonomies Version:
 *   2.0.2 Version description: Renamed packages, checked translation. Author:
 *               Paul van Buuren Author URI:          https://wbvb.nl/ License:
 *               GPL-2.0+ License URI:
 *   http://www.gnu.org/licenses/gpl-2.0.txt Text Domain:
 *   ictuwp-plugin-conference Domain Path:         /languages
 */
...
add_action('plugins_loaded', ['ICTU_GC_conference', 'init'], 10);
...
if (!class_exists('ICTU_GC_conference')) :
    ...
    class ICTU_GC_conference {
        ...
        private function fn_ictu_gcconf_setup_actions() {
            ...
            add_action('init', [$this, 'fn_ictu_gcconf_register_post_types']);
            add_action('init', 'fn_ictu_gcconf_initialize_acf_fields');

            add_action('plugins_loaded', [
              $this,
              'fn_ictu_gcconf_load_plugin_textdomain',
            ]);
            add_action('init', [$this, 'fn_ictu_gcconf_add_rewrite_rules']);

            // make sure the breadcrumb is useful for our CPTs
            add_filter('genesis_single_crumb', [
              $this,
              'fn_ictu_gcconf_filter_breadcrumb',
            ], 10, 2);
            ...
            add_filter('theme_page_templates', [
              $this,
              'fn_ictu_gcconf_add_page_templates',
            ]);

            // activate the page filters
            add_action('template_redirect', [
              $this,
              'fn_ictu_gcconf_frontend_use_page_template',
            ]);

            // add styling and scripts
            add_action('wp_enqueue_scripts', [
              $this,
              'fn_ictu_gcconf_register_frontend_style_script',
            ]);
            ...
            if (have_rows('extra_info_repeater', $post->ID)) {
                ...
                while (have_rows('extra_info_repeater', $post->ID)) : the_row();
                    ...
                    if ('video' === $type && $url) {
                        ...
                        if ($url && $linktext) {
                            if ($desc) {
                                $return .= '<li class="extralinks__item ' . $type . '">';
                                $return .= '<h3><a href="' . $url . '">' . $linktext . '</a></h3><p>' . $desc . '</p>';
                                $return .= '</li>';
                            }
                            else {
                                $return .= '<li class="extra-link-item ' . $type . '"><a class="extra-link extra-link--'. $type .'" href="' . $url . '"' . $download_attribute . '>' . $linktext . '</a></p>';
                            }
                        }
        ...
        public function fn_ictu_gcconf_register_frontend_style_script() {
            ...
            wp_enqueue_style(ICTU_GC_CONF_ARCHIVE_CSS, trailingslashit(plugin_dir_url(__FILE__)) . 'css/frontend-conf.css', [], ICTU_GC_CONF_VERSION, 'all');
        ...
        public function fn_ictu_gcconf_frontend_use_page_template() {
            ...
            if ($this->template_conf_overviewpage == $page_template) {
                ...
                add_filter('genesis_attr_entry', [
                  $this,
                  'fn_ictu_gcconf_add_class_inleiding_to_entry',
                ]);
            ...
            elseif (
              ...
              (is_tax(ICTU_GCCONF_CT_TIMESLOT))
                ...
                add_action('genesis_loop', [$this, 'fn_ictu_gcconf_tax_loop']);
            ...
            elseif (is_singular(ICTU_GCCONF_CPT_SESSION)) {
                ...
                add_action('genesis_entry_content', [
                  $this,
                  'fn_ictu_gcconf_frontend_append_speakers',
                ], 14);
            ...
            add_filter('genesis_post_info', [
              $this,
              'fn_ictu_gcconf_frontend_filter_postinfo',
            ], 10, 2);
        ...
        public function fn_ictu_gcconf_frontend_speaker_append_country() {
            ...
            if ($jobtitle || $country) {
                ...
                $return = '<div class="meta-data">';

                ($jobtitle ? $return .= '<span class="meta-data__item">' . $jobtitle . '</span>' : '');
                ($country ? $return .= '<span class="meta-data__item">' . $country . '</span>' : '');
        ...
        public function fn_ictu_gcconf_frontend_speaker_append_weblinks() {
            ...
            global $post;
            echo $this->fn_ictu_gcconf_frontend_speaker_append_links_sessions_keynotes([
              'ID' => $post->ID,
              'echo' => FALSE,
        ...
        public function fn_ictu_gcconf_frontend_write_keynotecard($args = []) {
            ...
            $return .= '<a href="' . get_permalink($args['ID']) . '" class="arrow-link"><span class="arrow-link__text">' . $section_title . '</span><span class="arrow-link__icon"></span></a></' . $args['titletag'] . '>';
            ...
            if ($list_of_speakers && $args['speakerimage']) {
                ...
                foreach ($list_of_speakers as $speaker):
                    ...
                    if (has_post_thumbnail($speaker)) {
                        $return .= get_the_post_thumbnail($speaker, SPEAKER_IMG_SIZE, ['class' => 'card__image']);
                    }
                    else {
                        //
                        $arr_speaker_images = get_field('fallback_for_speaker_images', 'option');
                        $randomid = array_rand($arr_speaker_images, 1);
                        $return .= wp_get_attachment_image($arr_speaker_images[$randomid], SPEAKER_IMG_SIZE, FALSE, ['class' => 'card__image']);
                    }
                ...
                if ($speakernames || $metainfo_time) {
                    $return .= $speakernames . $metainfo_time;
                }
            ...
            if (have_rows('extra_info_repeater', $args['ID'])) {
                ...
                while (have_rows('extra_info_repeater', $args['ID'])) : the_row();
                    ...
                    if ($url && $linktext) {
                        ...
                        $return .= '<' . $itemtag . ' class="extra-link extra-link--'. $type . '"><a href="' . $url . '"' . $download_attribute . '>' . $linktext . '</a></' . $itemtag . '>';
		...
		if ($metainfo) {
        $return .= '<div class="meta-data">' . $metainfo . '</div>';
    }

		$excerpt = get_the_excerpt($args['ID']);
		$return .= '<p class="card__description">' . wp_strip_all_tags($excerpt) . '</p>';
        ...
        if ($url && $linktext) {
            $return .= '<' . $itemtag . ' class="extra-link-item"><a class="extra-link extra-link--'. $type .'" href="' . $url . '">' . $linktext . '</a></' . $itemtag . '>';
        }
        ...
        public function fn_ictu_gcconf_frontend_write_speakercard($args = []) {
            ...
            $extraattr = ['class' => 'speaker-thumbnail thumbnail alignleft author-photo'];
            ...
            $return = '<div class="' . $args['cardcss'] . '" id="' . $title_id . '">' . $image;
            $return .= '<div class="speaker-info">';
            $return .= '<' . $args['titletag'] . ' class="author-title">';

            if ($args['titlelink']) {
                $return .= '<a href="' . get_permalink($args['ID']) . '" class="arrow-link"><span class="arrow-link__text">';
                $return .= $section_title;
                $return .= '</span><span class="arrow-link__icon"></span>';
                $return .= '</a>';
            }
            else {
                $return .= $section_title;
        ...
        public function fn_ictu_gcconf_frontend_speaker_featured_image() {
            ...
            if (has_post_thumbnail($post)) {
                ...
                echo get_the_post_thumbnail($post, SPEAKER_IMG_SIZE, ['class' => 'speaker-thumbnail thumbnail alignright']);
                echo '</span>';
            }
        ...
        public function fn_ictu_gcconf_frontend_sessionkeynote_location_time() {
            ...
            if ($metainfo) {
                ...
                echo '<div class="meta-data">' . $metainfo . '</div>';
        ...
        public function fn_ictu_gcconf_register_post_types() {
            ...
            register_taxonomy(ICTU_GCCONF_CT_TIMESLOT, [
            ...
            $args = [
              ...
              "query_var" => TRUE,
              "rewrite" => [
                'slug' => ICTU_GCCONF_CT_SESSIONTYPE,
                'with_front' => TRUE,
              ],
              "show_admin_column" => FALSE,
              "show_in_rest" => FALSE,
            ...
            register_taxonomy(ICTU_GCCONF_CT_SESSIONTYPE, [ICTU_GCCONF_CPT_SESSION], $args);
            ...
            register_taxonomy(ICTU_GCCONF_CT_LEVEL, [ICTU_GCCONF_CPT_SESSION], $args);
            ...
            register_taxonomy(ICTU_GCCONF_CT_LOCATION, [
              ICTU_GCCONF_CPT_SESSION,
              ICTU_GCCONF_CPT_KEYNOTE,
            ], $args);
            ...
            register_taxonomy(ICTU_GCCONF_CT_COUNTRY, [ICTU_GCCONF_CPT_SPEAKER], $args);
